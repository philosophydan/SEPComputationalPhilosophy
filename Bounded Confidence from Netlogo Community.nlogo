  ...
  if (communication_regime = "HK (select all)") [
   ...
   if not extremist? [
     ifelse (original = true)
        [ set opinion aggregate ( filter [ ?1 -> abs( ?1 - last opinion-list) < eps ] [last opinion-list] of turtles ) ]
        [ set opinion aggregate ( filter [ ?1 -> abs( ?1 - opinion) < eps ] [opinion] of turtles ) ]
...
to-report colorcode [x max_x]
  ...
  report hsb ((190 - 190 * (x / max_x))*(360 / 255)) 100 100
end
...
"" "clear-plot"
PENS
"default" 1.0 0 -5825686 true "" "if (max_eps > min_eps) [\nplotxy min_eps 0\nforeach ( n-values 99 [ ?1 -> (?1 + 1) / 100 * (max_eps - min_eps) + min_eps ] ) [ ?1 -> plotxy ?1 ( ((?1 - min_eps) ^ (alpha - 1) * (max_eps - ?1) ^ (beta - 1)) / ( max_eps - min_eps ) ^ (alpha + beta - 1) ) ]\nplotxy max_eps 0\n]"
...
This model was converted to work with Netlogo 6+ by the automatic process in Netlogo 6.1.0. Some coloring details were updated manually to work with Netlogo web - Daniel J. Singer
@#$#@#$#@
default
true
0
