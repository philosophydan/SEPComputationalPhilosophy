...
;;   final-performance: this variable is set by go-full, which is used to run multiple
;;                      instances of the model. This variable represents the mean performance
;;                      from those runs
;;   final-performance-var: this is the variance in the final performance from above

globals [bandit-means bandit-vars net-gap final-performance final-performance-var]
...
to setup
  ...
  ;; move the values from the interface into easier to use lists
  set bandit-means (list b1-mean b2-mean b3-mean b4-mean)
...
to go-full
  ...
  repeat Number-Of-Trials [
    ...
    set results lput (mean [my-performance] of turtles) results
  ]
...
to step
  if ticks > Number-Of-Pulls [
    set final-performance mean [my-performance] of turtles
    stop
  ]
  ask turtles [
    play
  ...
  ;; Depending on the density of the network there are two methods for updating
  ;; in addition, because the complete graph is so slow, we have a special updater for that

  ;; As you add networks: if the network is relatively sparse, use update-network-small
  ;; if the network is dense, use update-network-big.  Both work fine, but they have different
  ;; efficiency properties.  BUT NOTE: every turtle on a given round must use the same update
...
to update-network-small
  ...
  ask (turtle-set link-neighbors self) [
    ...
    let new-mean ((old-n * old-mean) + updating-result) / (old-n + 1)
...
to update-complete
  ...
  foreach n-values Number-of-Bandits [i -> i] [
    i ->
    let results [current-result] of (turtles with [current-bandit = i])
    if not empty? results [
      let mperf mean results
      let num length results
...
;;
;; draw-bandit [arm]: returns one draw of bandit arm
;;
...
to-report draw-bandit [arm]
  ...
  report random-normal item arm bandit-means item arm bandit-vars
end

;;
;; my performance: must be called in turtle context. reports how I've done so far.
;;
...
;;
;; network-gap: runs two simulations one with the complete graph and one with the
;;              cycle and sets the global net-gap with the difference in performance
;;              intended to be used to intellegently search the parameter space to find
;;              largest or smallest gaps
;;
...
to network-gap
  ...
  setup
  repeat Number-Of-Pulls [step]
  let complete-perf mean [my-performance] of turtles

  set Network "Cycle"
  setup
  repeat Number-Of-Pulls [step]
  let cycle-perf mean [my-performance] of turtles
...
"" ""
PENS
"default" 1.0 0 -16777216 true "" "plotxy ticks mean [my-performance] of turtles"
...
This model allows for the exploration of learning in bandit problems with individuals implementing variations on the epsilon-greedy learing rule.  The model allow one to explore how individuals learn on their own as well as in groups structured in different types of network structures.

## HOW IT WORKS

The underlying learning problem is a 2- to 4-armed bandit problem.  Each bandit is characterized by a mean payoff and variance.  A single pull of a bandit arm returns a value drawn from a normal distribution with those parameters.  

Individuals start off with some initial sample from each arm (specified by parameter INITIAL-SAMPLE).  They then, with probability 1-EPSILON, pull the arm that has performed best among the results that they have seen.  With probability EPSILON, they pull from among all the bandits uniformly at random.

Agents reevaluate their choice of arm after a set number of rounds to determine if another does better.  This is specified with the REEVAL-FREQUENCY paramater.  Agents continue for a set number of rounds, specified in the NUMBER-OF-PULLS parameter at which point the simulation stops.

All individuals can "see" their own results from each pull of the bandit. If the simulation involves more than one individual, they can be arranged in different network configurations.  In this case, individuals also see the results of others to whom they have a connection.  This model only features undirected networks, where results are transmitted in both directions.

## HOW TO USE IT

To run the model click the SETUP BUTTON. STEP has all players choose and pull one arm.  The GO button runs the simulation until all agents have pulled up to NUMBER-OF-PULLS.
...
@#$#@#$#@
<experiments>
  <experiment name="BothUnknown-IndivEpsVarianceSearch" repetitions="1" sequentialRunOrder="false" runMetricsEveryStep="false">
    <setup>set b2-variance b1-variance
setup</setup>
    <go>go-full</go>
    <timeLimit steps="1"/>
    <metric>b1-variance</metric>
    <metric>b2-variance</metric>
    <metric>final-performance</metric>
    <metric>final-performance-var</metric>
    ...
    <enumeratedValueSet variable="Reeval-Frequency">
      <value value="1"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="Visuals?">
      <value value="false"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="Number-Of-Trials">
      <value value="10000"/>
    </enumeratedValueSet>
  </experiment>
  <experiment name="BothUnknown-CycleComplete-Small" repetitions="1" sequentialRunOrder="false" runMetricsEveryStep="false">
    <setup>set b2-variance b1-variance
setup</setup>
    <go>go-full</go>
    <timeLimit steps="1"/>
    ...
    <metric>final-performance</metric>
    <metric>final-performance-var</metric>
    <steppedValueSet variable="Number-Of-Agents" first="3" step="10" last="53"/>
  ...
  <experiment name="BothUnknown-CycleComplete-Big" repetitions="30000" sequentialRunOrder="false" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>step</go>
    <metric>mean [my-performance] of turtles</metric>
    <steppedValueSet variable="Number-Of-Agents" first="50" step="10" last="150"/>
    <enumeratedValueSet variable="Epsilon">
      <value value="0"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="Initial-Sample">
  ...
  <experiment name="BothUnknown-CycleCompleteEps" repetitions="30000" sequentialRunOrder="false" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>step</go>
    <metric>mean [my-performance] of turtles</metric>
    <enumeratedValueSet variable="Number-Of-Agents">
    ...
    <steppedValueSet variable="Epsilon" first="0" step="0.025" last="0.5"/>
    <enumeratedValueSet variable="Initial-Sample">
      <value value="1"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="Network">
  ...
  <experiment name="SimpleTest" repetitions="3" sequentialRunOrder="false" runMetricsEveryStep="false">
    <setup>set b2-variance b1-variance
setup</setup>
    <go>go-full</go>
    <timeLimit steps="1"/>
