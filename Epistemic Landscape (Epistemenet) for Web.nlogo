...
to network-bit
  ...
  while [count nodes with [color = red] < count nodes]
  [
    ask one-of nodes with [color != red] [
      create-link-with one-of nodes with[color = red]
    ]
    find-all-components
    color-giant-component
  ]
  ...
  do-utility-plot
  cluster-coeff
  ;set average-path-length nw:mean-link-path-length turtles links
  ;set average-path-length2 network:mean-link-path-length nodes with [explored?] links
...
;;;;;;;;;;;;;;
;;; Layout ;;;  Layout Code Copyright 2005 by Uri Wilensky.  All rights reserved.
;;;;;;;;;;;;;;
;; resize-nodes, change back and forth from size based on degree to a size of 1
to resize-nodes
  ask nodes [ set size .1 + 3 * (utility / 800)]
end
...
365
363
% Converged on Highest
finalfinalconvergedonhighest
17
1
11
